...
module.exports = {
    getWalletDetail : "select u.id as user_id,'' as public,'' as private,coalesce(sum(t.token),0) as balance,coalesce(sum(t.amount),0) as inr_balance from users as u left join transaction as t on t.user_id=u.id and t.status=1 and transaction_type_id in (15,13,1,8) where u.id=? group by u.id,public",
