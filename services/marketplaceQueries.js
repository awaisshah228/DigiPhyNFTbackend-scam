...
module.exports = {
   ...
   itemCategory:"Select i.*,case when length(i.name)>=30 then concat(left(i.name,30),'...')  else i.name end as name,ie.id as item_edition_id,i.price,ie.is_sold,ic.name as category_name,ie.edition_text from item_edition as ie left join item as i on i.id=ie.item_id left join item_category as ic on i.item_category_id=ic.id where i.item_category_id = ? and ie.id !=? and i.is_active=1 and ie.id in (select min(id) from item_edition where is_sold=0 group by item_id) limit 3 ",
    ...
    getMarketActivity : "select t.id as transaction_id,case when tt.id=6 then 'Purchased for ' when tt.id=14 then 'Transfer NFT for' when tt.id=4 then 'Placed an offer for ' when tt.id=2 then 'Accepted bid offer for  ' when tt.id=9 then 'Published for resell ' end as transaction_type,abs(t.amount) as amount, case when i.edition_type=2 then 'Open'  else concat('Edition : ',coalesce(t.edition_text,ie.edition_text)) end as edition_text,date_format(t.datetime,'%d %M %Y') as transaction_date,t.item_id,t.transferNft,i.name,i.image,u.user_name,u.profile_pic,u.id as user_id From transaction as t left join transaction_type as tt on tt.id=t.transaction_type_id left join item_edition as ie on ie.id=t.item_edition_id left join item as i on i.id =ie.item_id left join users as u on u.id=t.user_id where i.id=? and transaction_type_id in (2,4,9,6,14) and t.status=1 union ALL select 0 as transaction_id,'Minted NFT for ' as transaction_type,i.price,case when i.edition_type=2 then 'Open'  else concat('Editions : ',getEditionCount(i.id)) end as edition_text,date_format(i.datetime,'%d %M %Y') as transaction_date,i.id as item_id,0,i.name,i.image,u.user_name,u.profile_pic,u.id as user_id from item as i left join users as u on u.id=i.created_by where i.id=? order by transaction_id desc",
