...
module.exports = {
    getWalletDetail : "select u.id as user_id,'' as public,'' as private,coalesce(sum(t.token),0) as balance,coalesce(sum(t.amount),0) as inr_balance from users as u left join transaction as t on t.user_id=u.id and t.status=1 and transaction_type_id in (15,13,1,8,12,3,17) where u.id=? group by u.id,public",
    getWalletTransaction :"SELECT t.id as transaction_id,tt.name as transaction_type,t.transaction_type_id,u.full_name,t.to_address,i.id as item_id,t.item_edition_id,i.name as item_name,i.file_type,i.description,i.image,COALESCE(abs(t.amount),0) as amount,COALESCE(abs(t.token),0) as token, t.currency,date_format(t.datetime,'%d-%M-%y') as transaction_date  FROM transaction as t left join users as u on u.id=t.user_id left join item_edition as ie on ie.id=t.item_edition_id left join item as i on i.id=ie.item_id left join transaction_type as tt on tt.id=t.transaction_type_id where t.user_id=? and t.transaction_type_id in (1,3,10,8,12,13,17) and (t.amount<>0 or token<>0) order by t.id desc",
    insertBuyTransactionByItemId : "INSERT INTO transaction (user_id,item_id,item_edition_id,token,transaction_type_id,amount,status,user_address,blockchain_status)select ?,i.id,ie.id,?,?,? as price,1,?,0 from item_edition as ie left join item as i on i.id=ie.item_id where ie.id =?",
    ...
    getUserOwnerItem :"select distinct u.id as user_id,uc.contractAddress,ie.isClaimed,ie.is_on_sale,t.id as transaction_id,ie.is_sold,i.is_minted,ie.owner_id,i.created_by,t.user_address,u.full_name,u.email,getUnclaimedNFTCount(i.id,t.user_id) as unclaimedNFT,uc.name as collection_name,i.id as item_id,ie.id as item_edition_id,case when i.edition_type=2 then 'Open'  else ie.edition_text end as edition_text,i.token_id, i.edition_type,i.name as item_name,i.description,i.image,getAvailableForResale(t.id,t.user_id) as totalStock,i.sell_type,i.bulkNFT,i.file_type,abs(COALESCE(t.amount,0)) as price,case when i.sell_type=1 then 'Open Edition' else 'Auction' end as bid_type,date_format(i.datetime,'%d-%M-%y') as nft_datetime,date_format(t.datetime,'%d-%M-%y') as purchase_datetime,concat('https://mumbai.polygonscan.com/tx/',ie.transfer_hash) as transfer_hash,cu.user_name as creator,coalesce(t.purchased_quantity,1) as purchased_quantity,case when i.edition_type=2 then 'Open' when t.edition_text is not null then t.edition_text  else ie.edition_text end as edition_text,getAvailableForResale(t.id,t.user_id) as resale_available,t.blockchain_status from transaction as t left join item_edition as ie on ie.id=t.item_edition_id left join item as i on i.id=ie.item_id left join users as u on u.id=i.created_by left join users as cu on cu.id=i.created_by  left join user_collection as uc on uc.id = i.user_collection_id where t.user_id=? and t.transaction_type_id in (6,2,14,16) and t.status<>3 and ie.owner_id=?  order by t.id desc",
   ...
   itemCategory:"Select i.*,case when length(i.name)>=30 then concat(left(i.name,30),'...')  else i.name end as name,ie.id as item_edition_id,i.price,ie.is_sold,ic.name as category_name,ie.edition_text from item_edition as ie left join item as i on i.id=ie.item_id left join item_category as ic on i.item_category_id=ic.id where i.item_category_id = ? and ie.id !=? and i.is_active=1 and ie.id in (select min(id) from item_edition where is_sold=0 group by item_id) limit 3 ",
    ...
    getMarketActivity : "select t.id as transaction_id,case when tt.id=6 then 'Purchased for ' when tt.id=14 then 'Transfer NFT for' when tt.id=4 then 'Placed an offer for ' when tt.id=2 then 'Accepted bid offer for  ' when tt.id=9 then 'Published for resell ' end as transaction_type,abs(t.amount) as amount, case when i.edition_type=2 then 'Open'  else concat('Edition : ',coalesce(t.edition_text,ie.edition_text)) end as edition_text,date_format(t.datetime,'%d %M %Y') as transaction_date,t.item_id,t.transferNft,i.name,i.image,u.user_name,u.full_name,u.profile_pic,u.id as user_id From transaction as t left join transaction_type as tt on tt.id=t.transaction_type_id left join item_edition as ie on ie.id=t.item_edition_id left join item as i on i.id =ie.item_id left join users as u on u.id=t.user_id where i.id=? and transaction_type_id in (2,4,9,6,14) and t.status=1 union ALL select 0 as transaction_id,'Minted NFT for ' as transaction_type,i.price,case when i.edition_type=2 then 'Open'  else concat('Editions : ',getEditionCount(i.id)) end as edition_text,date_format(i.datetime,'%d %M %Y') as transaction_date,i.id as item_id,0,i.name,i.image,u.user_name,u.full_name,u.profile_pic,u.id as user_id from item as i left join users as u on u.id=i.created_by where i.id=? order by transaction_id desc",
    ...
    getTransactionDetail1  : "Select i.name as item_name,i.edition_type,case when i.edition_type=2 then 'Open' when t.edition_text is not null then t.edition_text else ie.edition_text end as edition_text,i.image,i.file_type,case when ie.transfer_hash is not null then concat('https://mumbai.polygonscan.com/tx/',ie.transfer_hash) else t.receipt_url end as transfer_hash,u.user_name,u.full_name,u.profile_pic,t.*,ie.price  as amount,date_format(coalesce(t.datetime,now()),'%d-%M-%y %H:%i:%s') as datetime,case when t.status=2 then 'Rejected' when t.status=1 then 'Completed' else 'Pending' end as status,tc.name as type_name from item_edition as ie left join transaction as t on ie.id=t.item_edition_id  LEFT JOIN users as u ON ie.owner_id=u.id left join item as i ON i.id=ie.item_id LEFT JOIN transaction_type as tc ON t.transaction_type_id=tc.id where ie.id=? ORDER BY t.id DESC limit 1",
