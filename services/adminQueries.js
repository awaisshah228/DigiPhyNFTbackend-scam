...
module.exports = {
  ...
  getAdminItem: "Select users.user_name as owner_name, users.full_name as owner_name,i.id,ie.id as item_edition_id,ie.edition_text,cu.user_name as creator_name,cu.full_name as creator_name,i.name,ie.is_on_sale,i.description,i.royalty_percent,i.sell_type,i.user_collection_id,i.approve_by_admin,i.address,i.image,i.is_featured,i.owner_id,i.created_by,i.file_type,i.owner,i.item_category_id,i.quantity,ic.name as item_category,uc.name as collection_name,i.token_id,ie.price,i.is_active,ie.is_sold,ie.expiry_date from item_edition as ie left join item as i on i.id=ie.item_id left join item_category as ic on ic.id=i.item_category_id left join user_collection as uc on uc.id = i.user_collection_id left join users as cu on cu.id=i.created_by left JOIN users ON i.owner_id=users.id where ie.id in (select min(id) from item_edition where is_sold=0 group by item_id,owner_id)  ORDER BY i.id DESC",
